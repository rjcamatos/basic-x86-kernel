...
void idt_exception_handler(uint32_t vector, idt_registers_t regs, idt_error_t error, idt_cpu_frame_t cpu)
{
	const char* vinfo[] = {
	...
	if(vector > 31) {
		printk("\n");
		printk("Kernel Panic! Error Unknown Exception!\n");
		printk("Expception Vector: %u\n",vector);
		while(1) {
			//__asm__ volatile("cli; hlt");
			__asm__ volatile("cli; hlt; jmp -1");  // ← Garante que hlt é repetido mesmo que uma NMI tente acordar o processador
		}
	}
	...
	printk(
		"CPU FRAME: eip 0x%x, cs 0x%x, eflags 0x%x\n",
...
void idt_interrupt_handler(uint32_t vector, idt_registers_t regs, idt_error_t error, idt_cpu_frame_t cpu)
{	
	printk("Int %u, esp 0x%x || ",vector,regs.esp);
	// END-OF-INTERRUPT
...
void idt_mask_interrupt(uint8_t vector)
{
	// Vector < 32 = CPU Hardware Exceptions, mask them generates Fault
	if( vector < 32 ) return;

    uint8_t irq = vector - 32;

    uint32_t value = ioapic_read(ioapic_base,IOAPIC_IOREDTBL_L(irq));
    value |= (1 << 16);
    ioapic_write(ioapic_base, IOAPIC_IOREDTBL_L(irq), value);
}

void idt_unmask_interrupt(uint8_t vector)
{
	// Vector < 32 = CPU Hardware Exceptions, mask them generates Fault
	if( vector < 32 ) return;
