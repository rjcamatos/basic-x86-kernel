...
void ioapic_init(void)
{
	...
	ioapic_id_t ioid;
	ioapic_cast(ioid) = ioapic_read(ioapic_base,IOAPIC_IOAPICID);

	ioapic_ver_t iov;
	ioapic_cast(iov) = ioapic_read(ioapic_base,IOAPIC_IOAPICVER);
	...
	for( ;idx < iov.mre; idx++ )
	{
		...
		ioapic_write(ioapic_base, IOAPIC_IOREDTBL_L(idx),ioapic_cast(tbll));
		ioapic_write(ioapic_base, IOAPIC_IOREDTBL_H(idx),ioapic_cast(tblh));
	}
}
